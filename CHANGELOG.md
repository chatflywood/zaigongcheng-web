...
## 未发布：P0 数据版本与期间（2026-09-09）

- 同名上传与目标调整新增版本，历史快照保留；目标调整同步重新计算进度与差额。
- 上传支持独立业务日期；当前数据按业务日期选择，补传旧数据不替换当前期间。
- 预算联动保存工程来源版本，限制同年且不晚于预算日期；刷新仅返回实时派生结果，不改写快照。
- 页面显示日期及来源不一致提示；历史明细、导出沿用所查看版本的记录 ID。
- 新增 Excel 上传至历史回看的 P0 集成回归与前端状态测试，提供 scripts/check.sh 一键验证。
- 修复备份 API mock 缺失与转固目标组件类型警告，建立独立 Python 测试环境。
