<template>
  <div class="dashboard">
    ...
    <div v-show="batchTab === 'batch'" class="batch-view">
      ...
      <!-- ── 批次管理弹窗 + 专业管理抽屉（子组件） ── -->
      <BatchManageModal
        :modal-visible="batchModal.visible"
        :modal-mode="batchModal.mode"
        :modal-form="batchModal.form"
        :note-fields-open="noteFieldsOpen"
        :specialties="batchData.specialties"
        :specialty-visible="specialtyPanel"
        :specialty-list="specialtyList"
        :editing-specialty="editingSpecialty"
        :new-specialty-name="newSpecialtyName"
        :modal-subtotal="modalSubtotal"
        :modal-loading="modalLoading"
        @update:modal-visible="batchModal.visible = $event"
        @update:specialty-visible="specialtyPanel = $event"
        @update:editing-specialty="editingSpecialty = $event"
        @update:new-specialty-name="newSpecialtyName = $event"
        @close-batch-modal="closeBatchModal"
        @set-amount="setAmount"
        @toggle-note-field="toggleNoteField"
        @save-batch="saveBatch"
        @start-edit-specialty="startEditSpecialty"
        @save-specialty-edit="saveSpecialtyEdit"
        @add-new-specialty="addNewSpecialty"
        @confirm-delete-specialty="confirmDeleteSpecialty"
      />
...
import BatchManageModal from '../components/BatchManageModal.vue'
...
.table-card .progress-fill {
  background: linear-gradient(90deg, #245db0, #5f8fdd);
  border-radius: 999px;
}

.table-card .progress-fill.spend {
  background: linear-gradient(90deg, #b85a08, #e29b47);
}

.table-card .progress-fill.complete {
  background: linear-gradient(90deg, #047857, #32a37e);
}
