...
const isAnalystMode = computed(() => route.name !== 'key-indicators')
...
async function switchView(view) {
  ...
  if (view !== 'key-indicators') presentationMode.value = false
}
...
const { openZaigongSnapshot: openZaigongSnap, openBudgetSnapshot: openBudgetSnap } = useGlobalData()

async function openZaigongSnapshotAndNav(recordId) {
  await openZaigongSnap(recordId)
  router.push({ name: 'zaigong' })
  closeHistoryCenter()
}

async function openBudgetSnapshotAndNav(recordId) {
  await openBudgetSnap(recordId)
  router.push({ name: 'budget' })
  closeHistoryCenter()
}

function restoreAndGoLatest(sectionKey) {
  if (sectionKey === 'zaigong') {
    onZaigongRestoreLatest()
    router.push({ name: 'zaigong' })
  } else {
    onBudgetRestoreLatest()
    router.push({ name: 'budget' })
  }
}
    ...
    <aside class="app-sidebar" v-if="!presentationMode">
      ...
      <nav class="sidebar-nav">
        <div class="side-section">
          ...
          <div class="side-link"
            :class="{ active: currentRouteName === 'key-indicators', disabled: !canShowKeyIndicators }"
          ...
          <div class="side-link" :class="{ active: currentRouteName === 'zaigong' }" @click="switchView('zaigong')">
          ...
          <div class="side-link" :class="{ active: currentRouteName === 'budget' }" @click="switchView('budget')">
        ...
        <div class="side-section">
          ...
          <div class="side-link" :class="{ active: currentRouteName === 'archive' }" @click="switchView('archive')">
        ...
        <div class="side-section">
          ...
          <div class="side-link" @click="openDataManager">
            ...
            <span>数据管理</span>
            <span class="side-badge-dot" :class="{ ok: zaigongLatestData && budgetLatestData }"></span>
          </div>
          <div class="side-link" @click="openHistoryCenter">
            <svg class="side-icn" viewBox="0 0 16 16" fill="none"><path d="M2 5h12v9H2V5zM2 5V3h12v2M5 2v3M11 2v3" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>
            <span>上传历史</span>
          </div>
        ...
        <div class="dm-body">
          <div class="dm-card">
            <div class="dm-card-header">
              <div class="dm-card-title"><span class="dm-card-icon">①</span>在建工程明细总表</div>
              <div class="dm-card-header-r">
                <div class="dm-status" :class="zaigongLatestData ? 'status-ok' : 'status-none'">{{ zaigongLatestData ? '已加载' : '未上传' }}</div>
            ...
            <button class="dm-upload-btn" :disabled="dmZaigongLoading || !dmZaigongFileName" @click="dmUploadZaigong">{{ dmZaigongLoading ? '上传中…' : '上传在建工程数据' }}</button>
          </div>
          <div class="dm-card">
            <div class="dm-card-header">
              <div class="dm-card-title"><span class="dm-card-icon">②</span>预算执行情况（预算占用）</div>
              <div class="dm-card-header-r">
                <div class="dm-status" :class="budgetLatestData ? 'status-ok' : 'status-none'">{{ budgetLatestData ? '已加载' : '未上传' }}</div>
        ...
        <div v-else class="gh-body">
          <section v-for="section in historySections" :key="section.key" class="gh-section">
            <div class="gh-section-head">
              ...
              <button v-if="(section.key === 'zaigong' && zaigongSnapshotLabel) || (section.key === 'budget' && budgetSnapshotLabel)" class="gh-restore" @click="restoreAndGoLatest(section.key)">↩ 返回最新</button>
            </div>
            <div v-if="section.records.length === 0" class="gh-empty compact"><p>暂无记录</p></div>
            <div v-else class="gh-card-list">
              <button v-for="record in section.records" :key="`${section.key}-${record.id}`" class="gh-card" @click="section.key === 'zaigong' ? openZaigongSnapshotAndNav(record.id) : openBudgetSnapshotAndNav(record.id)">
                <div class="ghc-top">
            ...
            <div v-if="section.key === 'zaigong'" class="gh-compare">
              ...
              <div v-if="zaigongCompareResult" class="ghcmp-grid">
                <div class="ghcmp-card"><span>资本性支出</span><strong>{{ zaigongCompareResult.capitalCurrent.toFixed(2) }} 万</strong><p>{{ zaigongCompareResult.capitalDiff >= 0 ? '+' : '' }}{{ zaigongCompareResult.capitalDiff.toFixed(2) }} 万，完成率 {{ zaigongCompareResult.progressDiff >= 0 ? '+' : '' }}{{ zaigongCompareResult.progressDiff.toFixed(1) }} pct</p></div>
                <div class="ghcmp-card"><span>转固率</span><strong>{{ (zaigongCompareResult.rateCurrent * 100).toFixed(1) }}%</strong><p>变化 {{ zaigongCompareResult.rateDiff >= 0 ? '+' : '' }}{{ zaigongCompareResult.rateDiff.toFixed(1) }} pct</p></div>
              </div>
              <div v-if="zaigongCompareResult" class="ghcmp-table">
                <div class="ghcmp-table-head">管理员推进 Top 5</div>
                <table><thead><tr><th>管理员</th><th>当前</th><th>变化</th></tr></thead>
                  <tbody><tr v-for="item in zaigongCompareResult.top5" :key="item.name"><td>{{ item.name }}</td><td>{{ item.current.toFixed(2) }}</td><td :class="item.diff >= 0 ? 'delta-up' : 'delta-down'">{{ item.diff >= 0 ? '+' : '' }}{{ item.diff.toFixed(2) }}</td></tr></tbody>
                </table>
              </div>
            </div>
            <div v-if="section.key === 'budget'" class="gh-compare">
              ...
              <div v-if="budgetCompareResult" class="ghcmp-grid">
                <div class="ghcmp-card"><span>年度预算</span><strong>{{ budgetCompareResult.budgetCurrent.toFixed(2) }} 万</strong><p>变化 {{ budgetCompareResult.budgetDiff >= 0 ? '+' : '' }}{{ budgetCompareResult.budgetDiff.toFixed(2) }} 万</p></div>
                <div class="ghcmp-card"><span>立项进度</span><strong>{{ (budgetCompareResult.progressCurrent * 100).toFixed(1) }}%</strong><p>变化 {{ budgetCompareResult.progressDiff >= 0 ? '+' : '' }}{{ budgetCompareResult.progressDiff.toFixed(1) }} pct</p></div>
              </div>
              <div v-if="budgetCompareResult" class="ghcmp-table">
                <div class="ghcmp-table-head">专业推进 Top 5</div>
                <table><thead><tr><th>专业</th><th>当前</th><th>变化</th></tr></thead>
                  <tbody><tr v-for="item in budgetCompareResult.top5" :key="item.name"><td>{{ item.name }}</td><td>{{ (item.currentProgress * 100).toFixed(1) }}%</td><td :class="item.diff >= 0 ? 'delta-up' : 'delta-down'">{{ item.diff >= 0 ? '+' : '' }}{{ item.diff.toFixed(1) }} pct</td></tr></tbody>
                </table>
              </div>
            </div>
          </section>
      ...
      <div class="notify-modal">
        ...
        <div class="notify-modal-body">
          ...
          <div class="notify-field notify-toggle-row">
            <label>上传后自动推送</label>
            <label class="toggle-switch"><input type="checkbox" v-model="notifyAutoPush" /><span class="toggle-slider"></span></label>
          </div>
...
.app-shell.presentation-mode { display: block; }
.app-shell.presentation-mode .app-canvas { padding: 0; min-height: 100vh; background: #07111f; }

/* ── Sidebar ── */
.app-sidebar { position: sticky; top: 0; height: 100vh; display: flex; flex-direction: column; background: var(--paper-2); border-right: 1px solid var(--line); padding: 24px 18px; overflow-y: auto; z-index: 10; }
.sidebar-brand { display: flex; align-items: center; gap: 10px; padding: 0 6px; margin-bottom: 20px; }
.brand-mark { width: 28px; height: 28px; border-radius: 7px; background: var(--accent); color: #fff; font-family: var(--font-mono); font-size: 10px; font-weight: 500; letter-spacing: 0.05em; display: grid; place-items: center; flex-shrink: 0; }
.brand-name { font-size: 13px; font-weight: 500; color: var(--ink); letter-spacing: -0.005em; line-height: 1.2; }
.brand-sub { display: block; font-size: 11px; color: var(--ink-3); margin-top: 2px; font-weight: 400; }
.sidebar-nav { flex: 1; display: flex; flex-direction: column; gap: 28px; }
.side-section { display: flex; flex-direction: column; gap: 1px; }
.side-label { font-size: 10.5px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--ink-4); padding: 0 8px 6px; }
.side-link { display: flex; align-items: center; gap: 10px; padding: 7px 8px; border-radius: var(--r-md); font-size: 13px; color: var(--ink-2); cursor: pointer; user-select: none; transition: background 0.12s, color 0.12s; }
.side-link:hover:not(.disabled) { background: rgba(31,29,24,0.04); color: var(--ink); }
.side-link.active { background: rgba(31,29,24,0.06); color: var(--ink); font-weight: 500; }
.side-link.disabled { opacity: 0.4; cursor: default; }
.side-icn { width: 14px; height: 14px; flex-shrink: 0; opacity: 0.7; }
.side-link.active .side-icn { opacity: 1; }
.side-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--accent); margin-left: auto; flex-shrink: 0; }
.side-dot.ok { background: var(--ok); }
.side-wip { margin-left: auto; font-size: 10px; color: var(--ink-4); font-weight: 500; }
.side-badge-dot { width: 6px; height: 6px; border-radius: 50%; margin-left: auto; flex-shrink: 0; }
.side-badge-dot.ok { background: var(--ok); }
.sidebar-foot { margin-top: auto; padding-top: 14px; border-top: 1px solid var(--line); font-size: 11px; color: var(--ink-3); display: flex; flex-direction: column; gap: 4px; }
.sidebar-foot > strong { font-size: 12px; color: var(--ink-2); font-weight: 500; }
.side-version { color: var(--ink-4); margin-bottom: 4px; }

/* ── Canvas ── */
.app-canvas { min-width: 0; background: var(--paper); min-height: 100vh; overflow-y: auto; }
...
.gh-panel { width: min(600px, 100%); height: 100%; background: var(--surface); border-left: 1px solid var(--line); box-shadow: -8px 0 40px rgba(31,29,24,0.08); display: flex; flex-direction: column; overflow: hidden; }
.gh-head { display: flex; align-items: center; justify-content: space-between; padding: 16px 20px; border-bottom: 1px solid var(--line); flex-shrink: 0; }
.gh-head-left { display: flex; align-items: center; gap: 8px; color: var(--ink-3); }
.gh-head-left h3 { margin: 0; font-size: 15px; font-weight: 600; color: var(--ink); }
.gh-badge { padding: 1px 7px; border-radius: 999px; background: var(--paper-2); color: var(--ink-3); font-size: 11px; font-weight: 600; font-family: var(--font-mono); }
.gh-close { width: 28px; height: 28px; border-radius: var(--r-md); border: 1px solid var(--line); background: transparent; color: var(--ink-3); font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: 0.12s; }
.gh-close:hover { background: var(--paper-2); color: var(--ink); }
.gh-tabs { display: flex; gap: 2px; padding: 8px 12px; border-bottom: 1px solid var(--line); flex-shrink: 0; }
.gh-tabs button { height: 28px; padding: 0 12px; border-radius: var(--r-md); border: none; background: transparent; color: var(--ink-3); font: inherit; font-size: 12.5px; cursor: pointer; display: flex; align-items: center; gap: 4px; transition: 0.12s; }
...
.gh-body { flex: 1; overflow-y: auto; padding: 16px; display: flex; flex-direction: column; gap: 20px; }
.gh-empty { display: flex; align-items: center; justify-content: center; padding: 48px 20px; color: var(--ink-4); font-size: 13px; }
.gh-empty.compact { padding: 20px; }
.gh-trend { flex: 1; overflow-y: auto; padding: 16px; }
.gh-trend-head { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px; margin-bottom: 12px; }
.gh-trend-head h4 { font-size: 13px; font-weight: 600; color: var(--ink); margin: 0; }
.gh-legend { display: flex; align-items: center; gap: 12px; font-size: 11.5px; color: var(--ink-3); }
.gh-dot { display: inline-block; width: 7px; height: 7px; border-radius: 50%; margin-right: 4px; }
...
.gh-sec-left h4 { font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.07em; color: var(--ink-3); margin: 0; }
.gh-count { height: 16px; padding: 0 5px; border-radius: 999px; background: var(--paper-2); color: var(--ink-4); font-size: 10.5px; font-weight: 600; display: flex; align-items: center; font-family: var(--font-mono); }
.gh-restore { font-size: 11.5px; color: var(--accent); background: none; border: none; cursor: pointer; padding: 0; }
.gh-restore:hover { text-decoration: underline; }
.gh-card-list { display: flex; flex-direction: column; gap: 6px; }
.gh-card { display: flex; flex-direction: column; gap: 6px; width: 100%; text-align: left; padding: 10px 12px; background: var(--surface); border: 1px solid var(--line); border-radius: var(--r-lg); cursor: pointer; transition: 0.12s; }
...
.ghc-chip { font-size: 10.5px; padding: 1px 6px; border-radius: 4px; background: var(--paper-2); color: var(--ink-3); font-family: var(--font-mono); }
.ghc-kpis { display: flex; align-items: center; gap: 6px; padding-top: 4px; border-top: 1px dashed var(--line); }
...
.ghc-slot { height: 20px; min-width: 24px; padding: 0 6px; border-radius: 4px; border: 1px solid var(--line); background: var(--paper-2); color: var(--ink-4); font-size: 10.5px; font-weight: 700; cursor: pointer; transition: 0.12s; }
.ghc-slot.active { background: var(--ink); color: var(--surface); border-color: var(--ink); }
.gh-compare { padding: 12px; background: var(--paper-2); border: 1px solid var(--line); border-radius: var(--r-lg); display: flex; flex-direction: column; gap: 10px; }
...
.ghcmp-card { padding: 10px 12px; background: var(--surface); border: 1px solid var(--line); border-radius: var(--r-md); display: flex; flex-direction: column; gap: 2px; }
...
/* ── Notify modal ── */
.notify-overlay { position: fixed; inset: 0; background: rgba(31,29,24,0.4); backdrop-filter: blur(4px); z-index: 2000; display: flex; align-items: center; justify-content: center; }
.notify-modal { background: var(--surface); border: 1px solid var(--line); border-radius: var(--r-xl); width: 480px; max-width: 95vw; box-shadow: var(--shadow-pop); }
.notify-modal-header { display: flex; align-items: center; justify-content: space-between; padding: 14px 18px; border-bottom: 1px solid var(--line); }
.notify-modal-header h3 { margin: 0; font-size: 14px; font-weight: 600; color: var(--ink); }
.notify-modal-header .modal-close { background: none; border: none; font-size: 18px; color: var(--ink-4); cursor: pointer; line-height: 1; padding: 0 4px; }
.notify-modal-body { padding: 18px; display: flex; flex-direction: column; gap: 14px; }
.notify-status-row { display: flex; align-items: center; gap: 8px; font-size: 12.5px; color: var(--ink-3); }
.notify-badge { font-size: 10.5px; font-weight: 600; padding: 2px 7px; border-radius: 20px; }
...
.notify-masked { font-size: 11.5px; color: var(--ink-4); font-family: var(--font-mono); flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.notify-clear-btn { font-size: 11.5px; padding: 2px 9px; border: 1px solid var(--bad-soft); border-radius: 4px; background: transparent; color: var(--bad); cursor: pointer; flex-shrink: 0; }
...
.notify-btn-test { padding: 6px 14px; font-size: 12.5px; border: 1px solid var(--line-2); border-radius: var(--r-md); background: var(--surface); color: var(--ink-2); cursor: pointer; font-weight: 500; font-family: inherit; }
.notify-btn-test:hover:not(:disabled) { background: var(--paper-2); }
.notify-btn-save { padding: 6px 18px; font-size: 12.5px; border: none; border-radius: var(--r-md); background: var(--ink); color: var(--surface); cursor: pointer; font-weight: 600; font-family: inherit; }
